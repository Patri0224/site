...
export async function handler(event, context) {
  ...
  `;
    ...
    if (httpMethod === "DELETE") {
      if (queryStringParameters.rangeGiorni) {
        const { rangeGiorni, persona } = queryStringParameters;
        const days = parseInt(rangeGiorni);

        await sql`
                DELETE FROM public.calendar
                WHERE persona = ${persona}
              AND ripetibile = false
              AND data BETWEEN (CURRENT_DATE - INTERVAL '${days} days')
                          AND CURRENT_DATE;
        `;
        return { statusCode: 200, body: JSON.stringify({ success: true }) };
      } else {
